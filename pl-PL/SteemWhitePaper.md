...
Fundamentalną jednostką każdego konta platformy Steem jest STEEM, token w postaci kryptowaluty. Operacje na platformie Steem są oparte o system, w którym jeden token STEEM to jeden głos. W tym modelu, indywidua, które przyczyniają się dla platformy najbardziej, co można zmierzyć poprzez saldo konta, mają największy wpływ na to, jak wysoko oceniany jest inny wkład. Furthermore, Steem only allows members to vote with STEEM when it is committed to a vesting schedule. Under this model, members have a financial incentive to vote in a way that maximises the long term value of their STEEM.
