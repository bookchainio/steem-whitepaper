...
Na platformie Steem produkcja bloków odbywa się w systemie rundowym. Dla każdej rundy przydzielanych jest 21 świadków (z ang. witness - przyp. tłum.), aby stworzyć, a następnie podpisać blok transakcji. Dwudziestu z nich jest wybrana poprzez głosowanie (z ang. approval voting - przyp. tłum.). Ostatnie stanowisko jest współdzielone przez wszystkich strażników, którzy nie dostali się do finałowej dwudziestki, proporcjonalnie do ilości głosów, które uzyskali. Wspomnianych 21 aktywnych świadków jest losowanych co rundę po to, aby jeden strażnik nie musiał stale ignorować stworzonych wcześniej przez niego bloków. Każdy świadek, który opuścił blok i nie wyprodukował żadnego w ciągu ostatnich 24 godzin zostaje zablokowany do momentu w którym nie zaktualizuje jego klucza do podpisywania bloków (z ang. block signing key, przyp. tłum.).

Proces ten jest zaprojetowany tak, by dostarczać maksymalną wierzytelność przy jednoczesnym zachowaniu pewności, że każdy ma potencjał do produkcji bloków bez względu na jego popularność. Ludzie mają trzy opcje przezwyciężania cenzury narzuconej przez 20 wybranych strażników. Mogą cierpliwie poczekać w kolejce ze wszystkimi innymi użytkownikami, którzy nie należą do top 20 największych lub nabyć więcej SP, żeby zwiększyć swoją siłę głosu. Ogólnie rzecz biorąc, strażnicy, którzy stosują cenzurę, mogą łatwo stracić swoją pracę, dlatego wydaje się że cenzura nie będzie problemem dla platformy Steem.
