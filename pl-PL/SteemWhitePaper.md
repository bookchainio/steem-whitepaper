...
Z platformą Steem, produkcja bloków odbywa się w rundach. Dla każdej rundy przydzielanych jest 21 strażników (a ang. witness, przyp. tłum.), by stworzyć a następnie podpisać blok transakcji. Dwudziestu z nich jest wybrana poprzez głosowanie akceptacji (z ang. approval voting, przyp. tłum.). Ostatni natomiast jest współdzielony przez każdego strażnika, który nie wdostał się do rankingu 20 największych porównując proporcjonalnie ogólną ilość głosów. Wspomnianych 21 aktywnych strażników jest losowanych co rundę po to, aby jeden strażnik nie musiał stale ignorować stworzonych wcześniej przez niego bloków. Każdy strażnik, który opuścił blok i nie wyprodukował żadnego w ciągu ostanich 24 godzin zostaje zablokowany do momentu w którym nie zaktualizuje jego klucza do podpisywania bloków (z ang. block signing key, przyp. tłum.).

Proces ten jest zaprojetowany tak, by dostarczać maksymalną wierzytelność przy jednoczesnym zachowaniu pewności, że każdy ma potencjał do produkcji bloków bez względu na jego popularność. Ludzie mają trzy opcje przezwyciężania cenzury przez 20 wybranych strażników: cierpliwie poczekać w kolejce z każdym innym użytkownikiem, który nie należy do grupy 20 największych, lub nabyć więcej SP, by zwiększyć swoją siłę głosu. Ogólnie rzecz biorąc, stosowanie cenzury jest bardzo dobrą drogą dla wybranych strażników do utraty posady, czego konsekwencją jest ograniczenie ryzyka obarczenia sieci Steem tym czynnikiem do statystycznego minimum.
