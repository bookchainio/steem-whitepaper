...
Dolary Steem są stworzone za pomocą mechanizmu podobnego do convertible notes, które są często używane przy zbieraniu funduszy dla nowo-powstałych firm. W świecie startupów, convertible notes to instrumenty pozwalające na pożyczkę krótkoterminową, która może zostać przekonwertowana na udziały w firmie, jednak jej oprocentowanie, czy też współczynnik wymiany może zostać ustalony w późniejszym czasie, najczęściej przy następnej rundzie finansowania. A blockchain based token can be viewed as ownership in the community whereas a convertible note can be viewed as a debt denominated in any other commodity or currency. The terms of the convertible note allow the holder to convert to the backing token with a minimum notice at the fair market price of the token. Creating token-convertible-dollars enables blockchains to grow their network effect while maximizing the return for token holders.
