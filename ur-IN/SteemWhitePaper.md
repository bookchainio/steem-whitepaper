...
ایک بلاچین کم از کم بیلنس کے تقاضے کے سادہ طریقے کے ذریعے فی صارف کم از کم قیمت کو نافذ کر سکتا ہے. Any business that wishes to bring a new customer to the blockchain can pre-fund that user’s account with the minimum balance that would allow them to transact. Requiring a relatively large fee ($1.00) to sign up new users will naturally force anyone offering free accounts to vet the quality and uniqueness of each account before registering them with the blockchain.
