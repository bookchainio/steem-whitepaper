...
Blockchain temelli sosyal medyada ise bir üretici veya müellif, içerik kaynağının ispatını gösteren herkese açık bir kayda ve zaman damgasına atıf yapabilir. Herhangi bir içerik yaratıcısının kendi içeriğini izin almadan veya atıf yapmadan tekrar paylaşanlara hitap etmek istemesi durumunda, blockchain temelli kayıtlar sözkonusu içeriğin belirli bir kullanıcı tarafından belirli bir tarihte paylaşıldığına dair kamuya açık delil sunarlar. In the future, blockchain-based attribution could come to be recognized by governments for its authenticity and could hold weight in court, which would give content creators greater powers to control their work.
